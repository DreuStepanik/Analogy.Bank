...
function validatePIN (pin) {
  // add an attribute for the length of the pin.
  var pinLength = pin.length;
  // create an attribute to set a default to the length of the pin.
  var isCorrectLength = (pinLength == 4 || pinLength == 6);
  // create an attribute to verify that the pin only has numbers included.
  // (/^ == open... match) 
  // (\d == Matches a digit character. Equivalent to [0-9]) 
  // (+  == Matches the preceding character or subexpression one or more times.)
  //        For example, 'zo+' matches "zo" and "zoo", but not "z". + is equivalent to {1,}
  // ($/ == matches the end of the line.. closes the expression.)
  var hasOnlyNumbers = pin.match(/^\d+$/);
  
  // If the length is accurate, and the pin only contains digits, return true and proceed.
  if(isCorrectLength && hasOnlyNumbers){
    return true;
  }
  // If this does not meet the conditions, return false.
  return false;
}
